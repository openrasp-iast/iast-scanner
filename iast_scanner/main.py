...
def init_check():
    ...
    try:
        import aiohttp, aiomysql, jsonschema, lru, peewee, peewee_async, psutil, pymysql, tornado, yaml, requests
    ...
    try:
        ...
        if len(result) > 0 and int(result[0][1]) < 1000:
            print(
                "[!] Warning: MySQL max_connections is set to {}, this may limit maximum number of concurrent scan task, set max_connections at least to 1000 if you have large number of concurrent scan task.".format(
                    result[0][1]))
    ...
    if Config().config_dict["cloud_api.enable"]:
        ...
        for k, v in cloud_config.items():
            if v == "":
                print(
                    "[!] Config item cloud_api.{} is empty, refer to rasp cloud page to configure this option!".format(
                        k))
                sys.exit(1)
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if response["status"] != 0:
                    print(
                        "[!] Test cloud server failed, got HTTP code: {} with response: {} from cloud server: {}, check option startswith 'cloud_api' in config file!".format(
                            r.status_code, r.text, url))
                    sys.exit(1)
            else:
                print(
                    "[!] Test cloud server failed, got HTTP code: {} from cloud server: {}, check option startswith 'cloud_api' in config file!".format(
                        r.status_code, url))
                sys.exit(1)
            ...
            print("[!] Cloud server url:{} connect failed, check option startswith 'cloud_api' in config file!".format(
                url))
            sys.exit(1)
...
def show_version(args):
    print(f"iast-scanner_{open(os.path.join(os.path.dirname(__file__), 'VERSION')).read().strip()}")
