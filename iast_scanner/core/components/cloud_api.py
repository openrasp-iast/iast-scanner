...
from core.components.util import get_local_ip_addresses
...
class Transaction(object):
        ...
        while True:
            ...
            try:
                async with AioWebSocket(uri, union_header=union_header, timeout=5) as aws:
                    ...
                    # heartbeat monitor info
                    info = {
                        "app_id": self.app_id,
                        "monitor_url": f"http://{get_local_ip_addresses()}:{self.monitor_port}"
                    }
                    # 客户端给服务端发送消息
                    await converse.send(json.dumps(info))
                    self.message_bucket = []
                    success = True
                ...
                if not success:
                    Logger().warning("Lost connection with cloud server, will try to connect after 5 seconds!",
                                     exc_info=e)
    ...
    def run(self):
        remote = self.server_url.replace("https", "wss").replace("http", "ws") + "/v1/iast/order"
        union_header = {
