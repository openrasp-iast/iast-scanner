...
class Logger(object):
    ...
    def _init_error_log(self):
        ...
        # 文件输出
        file_handler = RotatingFileHandler(self.log_path + "/error.log", mode='a',
                                           maxBytes=Config().get_config("log.rotate_size") * 1024 * 1024,
                                           backupCount=Config().get_config("log.rotate_num"))
        file_formatter = logging.Formatter(
            '[%(asctime)s - %(levelname)s][%(processName)s] %(message)s [file: %(pathname)s , line %(lineno)d]',
            '%Y-%m-%d %H:%M:%S')
    ...
    def _set_handler(self, logger, suffix, log_fmt):
        """
        为logger配置Handler
        """
